...
if uploaded_file is not None:
    ...
    # ✨ ส่วนที่อัปเกรดใหม่: กำหนดวันที่เพื่อเช็ครอบรถและพิมพ์หน้าใบงาน
    st.subheader("📅 กำหนดวันที่จัดส่งจริง (บังคับใส่ เพื่อตรวจสอบรอบรถ)")
    st.info("💡 ระบบจะใช้วันที่นี้ในการพิมพ์หัวกระดาษ และจะคำนวณอัตโนมัติว่าตรงกับวันอะไร เพื่อดึงตารางรอบส่งมาเช็คครับ")
    
    # ดึงวันที่ปัจจุบันมาเป็นค่าเริ่มต้นให้พิมพ์ง่ายๆ
    ...
    if st.button("🚀 สร้างใบงานเดี๋ยวนี้", type="primary"):
        if len(filtered_df) == 0:
            ...
            try:
                # 1. แปลงข้อความวันที่ที่ผู้ใช้พิมพ์ เพื่อหาวันในสัปดาห์
                parsed_date = pd.to_datetime(target_delivery_date.strip(), format='%d/%m/%Y')
                day_name = parsed_date.day_name() # จะได้ชื่อวัน เช่น 'Monday', 'Tuesday'
                
                # แปลงชื่อวันเป็นภาษาไทยเพื่อแสดงผล
                ...
                # 2. นำชื่อวัน ไปเทียบหาในตารางรอบรถ
                if route_dict:
                ...
                with st.spinner(f"กำลังเช็ครอบรถของ วัน{day_name_thai} และจับคู่ใบงาน..."):
                    ...
                    def build_inner_tag(group_key, group_df, target_date_str):
                        ...
                        <div class="header">
                            ...
                            <div class="doc-info">วันที่จัดส่ง: {target_date_str}</div>
                        </div>
                        <div class="item-card">
                    ...
                    st.success(f"✅ ตรวจพบเป็น วัน{day_name_thai} เช็ครอบรถและสร้างใบงานเสร็จสมบูรณ์!")
                    st.download_button(
                ...
                st.error("❌ รูปแบบวันที่ไม่ถูกต้อง! กรุณาพิมพ์ในรูปแบบ วัน/เดือน/ปี เช่น 22/06/2026 (คั่นด้วย / เท่านั้น)")
